...
contract XStella is ERC20("XStella", "xStella") {
    ...
    // Firstly the attacker sends stellla to the msart contract without minting.... and 
    // and others will continue to mint some wei andd all..... 
    
        else {
            uint256 xStellaAmount = _stellaAmount.mul(totalShares).div(totalStella);
            // @question soo we can directly send totalStella  and mint small
            _mint(msg.sender, xStellaAmount);
        }
        // Lock the Stella in the contract
        stella.transferFrom(msg.sender, address(this), _stellaAmount);
    }
// so we increase the balanceOf(this) which consequwntly increase totalStella withoutt increasing the totalShares.
// therefore causing a discrepancy and allwing  this mul(totalShares).div(totalStella); to returnn zero  even is this is large enough _stellaAmount
// and the user (innoncet) mints 0. which causes a larger discrepancy because the balanceOf(this ) will icnrease again... after the innoncent user sends 
// but the totalShares which is = totalSuplly will not increase since we minted 0. leaving a larger discrepnacy.


function check(uint256 _ch) public view returns(uint256){
 (_ch.mul(9)).div(10);
}
...
// WE........... Pool Details to the left....  Join Trading Pool 
// Pool Tracker 
// Top Ranking pools people in the pool and amount people.....
// 
