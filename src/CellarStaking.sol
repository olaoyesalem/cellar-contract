...
contract CellarStaking is ICellarStaking, Ownable {
    ...
    function stake(uint256 amount, Lock lock) external override whenNotPaused updateRewards {
        // how is the longer locs recieve rewards boost..and how can we manipoulate.. short time but still the reqrd boost
        // which means user can stake without.. paying
        ...
        if (totalDeposits == 0 && rewardsReady > 0) {
            _startProgram(rewardsReady); // This can give us zero ....  and the rewards rate can give us 0.
            rewardsReady = 0;
        ...
        (uint256 boost, ) = _getBoost(lock); // this gets the amount to boost.
        ...
        stakingToken.safeTransferFrom(msg.sender, address(this), amount); /// This will never revrt
// the issue spawns from where the staking Token is udpatedd
...
// that means that there can be a time whre the `rewardPerToken` will not increase   because  `newRewardsPerToken` is 0.
//@bug       
