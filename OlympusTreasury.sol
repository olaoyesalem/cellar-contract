...
contract OlympusTreasury is Ownable {
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        IERC20Mintable( OHM ).mint( msg.sender, send_ ); // we get minted 0...
    ...
    // asuming the total supply is 100
    // totalReserve is 100 too
    // but we can continue to increase totalReserves without increasing the totalSupply.. 
    // what is the impact??
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        require( _amount <= excessReserves(), "Insufficient reserves" ); //@bug which means that we can increase the mint reward more than usual
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( OHM ).totalSupply().sub( totalDebt ) );  // this  is going to be greater than useual @bug
    }
